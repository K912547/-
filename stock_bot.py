...
import pandas as pd
# 關鍵：為了防止大小寫模組找不到的 Bug，我們這裡用 try-except 包起來保護
try:
...
async def on_message(message):
    ...
    if message.content.startswith('!查價 '):
        ...
        if (".TW" in ticker or ".TWO" in ticker) and HAS_FINMIND:
            try:
                ...
                chip_text = "❌ 籌碼面資料讀取失敗。"
        elif not HAS_FINMIND and (".TW" in ticker or ".TWO" in ticker):
            chip_text = "⚠️ 伺服器缺少 FinMind 元件，無法載入籌碼。"
